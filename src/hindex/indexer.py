...
def _cosine_search_numpy(xb: np.ndarray, q: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # cosine = dot because vectors are L2-normalized already
    scores = xb @ q.reshape(-1, 1)  # (n,1)
